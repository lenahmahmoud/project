...
function Cleansers({searchInput}) {
    // cleansers to be rendered
    const [cleansers, setCleansers] = useState([]);

    // template
    const [originalCleansers, setoriginalCleansers] = useState([]);
    ...
    useEffect(()=>{
        if(searchInput===" "){
         setCleansers(originalCleansers)
         setSearched([])
        }
        else{
           const filtered=originalCleansers.filter((p)=>{
            p.title.toLowerCase().includes(searchInput.toLowerCase())

           })
           setCleansers(filtered)
    ...
    const applyFilters = () => {
        let result
        searched.length>0? (result=[...searched]):(result=[...originalCleansers])
